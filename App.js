...
export default function App() {
  // Estados para armazenar a entrada do usuário e o resultado final
  const [inputValues, setInputValues] = useState("");
  const [sortedValues, setSortedValues] = useState("");

  // Função que implementa o algoritmo estruturado (Bubble Sort)
  const handleSort = () => {
    Keyboard.dismiss(); // Esconde o teclado ao processar

    // 1. Tratamento dos dados de entrada
    // Separa a string por vírgulas, remove espaços em branco e converte para números
    let arr = inputValues
      .split(",")
      .map((item) => Number(item.trim()))
      .filter((item) => !isNaN(item)); // Remove entradas inválidas (textos)
    ...
    // 2. Implementação do Algoritmo Bubble Sort (Ordenação por Bolha)
    let n = arr.length;
    let swapped;
    ...
    do {
      ...
      // Otimização do Bubble Sort: o último elemento já está no lugar certo
      n--;
    } while (swapped);

    // 3. Atualização da interface com o resultado
...
// Estilização básica e organizada do aplicativo
